...
export function LiveLeaderChip({ leader, subjectPlural = "players", metricLabel }: Props) {
    ...
    if (leader.kind === "unavailable") {
        ...
        return (
            <p className={`${baseClass} opacity-40`}>
                <span className="opacity-60">currently:</span> {reason}
            </p>
        );
    }
    ...
    if (leader.kind === "single") {
        return (
            <p className={baseClass}>
                <span className="opacity-50">currently:</span>{" "}
